...
async function seed() {
  ...
  await Promise.all([
    order[0].addProduct(cupcakeProduct, { through: { quantity: 2, priceWhenOrdered: cupcakeProduct.price } }),
    order[0].addProduct(cakeProduct, { through: { quantity: 1, priceWhenOrdered: cakeProduct.price } }),  
    order[1].addProduct(cookieProduct, { through: { quantity: 1, priceWhenOrdered: cookieProduct.price } }),
    order[1].addProduct(cupcakeProduct, { through: { quantity: 4, priceWhenOrdered: cupcakeProduct.price } }),
    order[2].addProduct(cookieProduct, { through: { quantity: 5, priceWhenOrdered: cookieProduct.price } }),
    order[2].addProduct(cakeProduct, { through: { quantity: 2, priceWhenOrdered: cakeProduct.price } }),  
    order[3].addProduct(cupcakeProduct, { through: { quantity: 1, priceWhenOrdered: cupcakeProduct.price } }),
    order[3].addProduct(cakeProduct, { through: { quantity: 3, priceWhenOrdered: cakeProduct.price } }),  
    order[4].addProduct(cookieProduct, { through: { quantity: 2, priceWhenOrdered: cookieProduct.price } })
  ])
