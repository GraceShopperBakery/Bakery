...
class UpdateProduct extends Component {
  ...
  async handleSubmit(evt) {
    ...
    if (this.state.title) {
      updatedProduct.title = this.state.title
    }
    if (this.state.description) {
      updatedProduct.description = this.state.description
    }
    if (this.state.imageURL) {
      updatedProduct.imageURL = this.state.imageURL
    }
    if (this.state.category.length > 1) {
      updatedProduct.category = this.state.category
    }
    if (this.state.price) {
      updatedProduct.price = this.state.price
    }
    if (this.state.inventoryQuantity) {
      updatedProduct.inventoryQuantity = this.state.inventoryQuantity
    }
  ...
  render() {
    ...
    const categories = this.props.product.categories
      ? this.props.product.categories.map(category => category.name).join(', ')
      : ''
        ...
        <form className="form" onSubmit={this.handleSubmit}>
          ...
          <div className="form-group">
            <label htmlFor="category">
              'Category (separate by comma if more than one)'
            </label>
            <div className="form-control">
              <input
                onChange={this.handleChange}
