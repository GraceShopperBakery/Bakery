...
class Shop extends Component {
  ...
  handleAddToCart(event) {
    const productToAdd = this.props.products.find(product => product.title === event.target.name)
      ...
      <div className='allProductsPage'>
        ...
        { currentProduct.map(product => (
          ...
          <figure className="product">
            ...
            <button type="button" name={product.title} onClick={this.handleAddToCart}>Add To Cart</button>    
          </figure>
          </div>
        ))}
        </div>
        <button type="button" onClick={this.showCart}>SHOW CART</button>
      </div>
